...
for i in range(len(df_rasio_int)):
    for j in range(len(df_rasio_int)):
        if df_rasio_int.at[i, 'Peak_time_ave'] == df_rasio_int.at[j, 'OxyTime']:
            ...
            # ===== 除外条件 =====
            # if 97 <= spo2 <= 100:
            #     if ratio < lower_limit_mad or ratio > upper_limit_mad:
            ##
            ##
            ##
            ##
...
sample_counts = df_new_cutted.groupby("Spo2_int").size().reset_index(name="n_samples")
sample_counts["weight"] = np.sqrt(sample_counts["n_samples"])
new_df_include_sample_num = df_new_cutted.merge(sample_counts, on="Spo2_int", how="left")
...
all_df=df_all_cutted.copy()
all_df['Spo2_smo']=all_df['Spo2'].rolling(window=movingAveragePoint).mean()

plt.figure(figsize=(10, 6))
plt.plot(all_df['Peak_time_ave'], all_df['Spo2_smo'], color='red', label='Camera', linewidth=2)
