...
for i, row1 in data1.iterrows():
    for j, row2 in data2.iterrows():
        # time1_col と time2_col の差の絶対値が 0.025 未満の場合
        if abs(row1[time1_col] - row2[time2_col]) < 0.025:
...
# ループを実行してデータを比較し、条件に合致する行を新しいデータフレームに追加する
    ...
    for j in range(len(df_rasio_int)):
        if df_rasio_int.at[i, 'Peak_time_ave'] == df_rasio_int.at[j, 'OxyTime']:
            new_data.append({
                ...
                'ratio_Peak-Peak': df_rasio_int.at[i, 'ratio_Peak-Peak'],
                #'OxyTime': df_rasio_int.at[j, 'OxyTime'],
...
with pd.ExcelWriter(output_file_name) as writer:
    ...
    new_df.to_excel(writer, sheet_name='relation_spo2')
