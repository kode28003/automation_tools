...
results = calc_spo2_ratio_range(slope_num, intercept, width)
...
subset_10_40 = df_new_cutted[(df_new_cutted['Peak_time_ave'] >= calibrationTimeStart) & (df_new_cutted['Peak_time_ave'] <= calibrationTimeEnd)].copy()
subset_10_40['b'] = subset_10_40['Spo2'] - slope_num * subset_10_40['ratio_Peak-Peak']
# 切片bの中央値
b_median = subset_10_40['b'].median()
print(" ")
...
print(f"キャリブレーション後の近似式: SpO2 = {slope_num:.2f} * ratio + {b_median:.2f}")
b_median=intercept
print(f"10〜40秒の切片中央値 切片固定 b: {b_median:.2f}")
print(f"キャリブレーション後の近似式 切片固定: SpO2 = {slope_num:.2f} * ratio + {b_median:.2f}")

# 切片bの中央値
cutted_median = subset_10_40_cutted['b'].median()
cutted_median = intercept
print(" ")
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f"C:/Users/mpg/Desktop/python_rasio/output_image/compSpO2_slope_{slope_num}_seppen_{b_median}.png")
plt.show()
...
plt.figure(figsize=(10, 6))
plt.plot(df_new_cutted_est['Peak_time_ave'], df_new_cutted_est['Spo2_est_smooth'], color='red', label=f'Camera (slope:{slope_num} , intercept:{b_median})', linewidth=3)
...
plt.tight_layout()
plt.savefig(f"C:/Users/mpg/Desktop/python_rasio/output_image/seppen_{b_median}.png")
plt.show()
...
plt.scatter(new_df['ratio_Peak-Peak'], new_df['Spo2'], label='cal data cut')
plt.xlabel('ratio', fontsize=16)
plt.ylabel('SpO2 [%]', fontsize=16)
plt.legend()
plt.xlim(0.8, 2.1) 
