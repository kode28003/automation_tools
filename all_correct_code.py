...
plt.xlim(0.8, 2.1)
plt.ylim(75, 102.5)
plt.savefig("C:/Users/mpg/Desktop/python_rasio/output_image/all_plotsssss.png")
plt.show()
...
new_df_with_est['Spo2_est_smooth'] = new_df_with_est['Spo2_est'].rolling(window=movingAveragePoint, center=True).mean()
...
df_new_cutted_est['Spo2_est_smooth'] = df_new_cutted_est['Spo2_est'].rolling(window=movingAveragePoint, center=True).mean()
...
df_new_cutted_est_base_slope_num['Spo2_est_smooth'] = df_new_cutted_est_base_slope_num['Spo2_est'].rolling(window=movingAveragePoint, center=True).mean()


plt.figure(figsize=(10, 6))
plt.plot(df_new_cutted_est['Peak_time_ave'], df_new_cutted_est['Spo2'], color='blue', label='PulseOxy', linewidth=2)
plt.plot(df_new_cutted_est['Peak_time_ave'], df_new_cutted_est['Spo2_est_smooth'], color='red', label=f'Camera (slope:{slope_num})', linewidth=2)
plt.plot(df_new_cutted_est_base_slope_num['Peak_time_ave'], df_new_cutted_est_base_slope_num['Spo2_est_smooth'], color='green', label=f'Camera (base slope:{base_slope_num})', linewidth=2, alpha=0.7)
plt.xlabel('Time (s)', fontsize=16)
plt.ylabel('SpO₂ (%)', fontsize=16)
